...
ROOT = r"D:\Portfolio\mesa\MesaRPG\modelsmw5\activos\Archer\Model"
BODY_DIR = ROOT + r"\Body" + os.sep
WEAPONS_DIR = ROOT + r"\Weapons" + os.sep
ANIM_DIR = r"D:\Portfolio\mesa\MesaRPG\modelsmw5\activos\Archer\Animation" + os.sep
OUT_PATH = r"D:\Portfolio\mesa\MesaRPG\models\Archer_new.blend"

# --- 1. Import body ---
bpy.ops.uf.import_uemodel(directory=BODY_DIR, files=[{"name": "Archer_SKM.uemodel"}])
...
print("Has Forearm_Left_Weapon:", "Forearm_Left_Weapon" in bone_names)
print("Has Torso_Head:", "Torso_Head" in bone_names)

# --- 3. Cockpit: SKIPPED (matches Annihilator's proven simpler approach) ---

# --- 4. Weapon mounting ---
BONE_BY_LOCATION = {
    "Forearm_Left": "Forearm_Left_Weapon",
    "Forearm_Right": "Forearm_Right_Weapon",
    "Torso_Center": "Torso_Weapon",
    "Torso_Left": "Torso_Weapon",
    "Torso_Right": "Torso_Weapon",
    "Torso": "Torso_Weapon",
}
LOCATION_HINT = {
    "Forearm_Left": "left_arm",
    "Forearm_Right": "right_arm",
    "Torso_Center": "center_torso",
    "Torso_Left": "left_torso",
    "Torso_Right": "right_torso",
    "Torso": "center_torso",
}

# Filename pattern: Weapon_Mech_ARC_<Location...>_<Slot>_<Visual>_SKM.uemodel
FILENAME_RE = re.compile(r"^Weapon_Mech_ARC_(.+)_SKM\.uemodel$", re.IGNORECASE)
...
for fname in weapon_files:
    ...
    # Plain zero-offset bone-parent (matrix_parent_inverse stays identity).
    root_obj.parent = armature
    root_obj.parent_type = "BONE"
    root_obj.parent_bone = target_bone

    hint = LOCATION_HINT[location]
    new_name = f"chrMdlWeap_Archer_{hint}_{visual}_{slot.lower()}"
    # BUG CRITICO #2 (ver doc): el nombre reconocido va en la MALLA, nunca
    # en el wrapper-armature (que seguiria empezando por chrMdlWeap_ y
    # duplicaria entradas fantasma). El wrapper se queda con un nombre
    # completamente ajeno a la convencion.
...
# --- 5. BUG CRITICO 12b: reset pose to TRUE rest before saving/exporting ---
bpy.context.view_layer.objects.active = armature
