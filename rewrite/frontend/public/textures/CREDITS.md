...
- `dice/dice-chrome-color.jpg` / `dice-chrome-normal.jpg` / `dice-chrome-roughness.jpg` / `dice-chrome-metalness.jpg` — [Metal032](https://ambientcg.com/view?id=Metal032), full PBR set (Color/NormalGL/Roughness/Metalness), downscaled from 1K to 512px — clean polished steel, Die.tsx's `chrome-metallic` die style's real scanned material (per explicit request: "dados metalicos con textura... PBR, mapa metalico, roughtness y glossiness, normales").
- `dice/dice-rust-color.jpg` / `dice-rust-normal.jpg` / `dice-rust-roughness.jpg` / `dice-rust-metalness.jpg` — [Metal025](https://ambientcg.com/view?id=Metal025), full PBR set, downscaled from 1K to 512px — genuinely oxidized/weathered steel (real photogrammetry, not a hand-painted mask layer), Die.tsx's `gunmetal-metallic` die style — this scan's own color/roughness variation IS the "mascara de oxido" ask.
- `../dice-env.exr` — [Day Sky HDRI 067 B](https://ambientcg.com/view?id=DaySkyHDRI067B), the real linear 1K `.exr` (not the tonemapped `sky.jpg` above, which is a separate, already-baked JPG export of the same source used for FirstPersonView's visible sky backdrop) — loaded via drei's `<Environment>` in TableView.tsx for real HDRI reflections on metallic/glass dice (lighting-only, `background={false}`, so it doesn't touch what's actually drawn behind the table). Same source asset as `sky.jpg`, just the un-tonemapped HDR variant this use actually needs.
