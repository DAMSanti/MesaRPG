...
# La asignación track_id -> sheet_id vive en game_state.miniature_assignments
# (única fuente de verdad, ver GameStateManager.assign_miniature/unassign_miniature).
...
async def assign_miniature_to_character(body: dict = Body(...)):
    ...
    assignments = game_state.assign_miniature(int(track_id), character_id)
...
async def unassign_miniature_from_character(track_id: int):
    ...
    return {"status": "unassigned", "assignments": assignments}
...
async def websocket_camera(websocket: WebSocket):
    ...
    async def process_latest_frame():
        ...
        try:
            ...
            if tracks:
                await ws_manager.send_miniature_positions(tracks, frame_processor.get_frame_size())
